...
import { Bell, Search, ChevronDown, Key, LogOut, X, BookOpen, Users, BedDouble, Clock, Sparkles, Wrench, CheckCircle2 } from 'lucide-react'
...
// ──────────────────────────────────────────────
// PinInputRow — with auto-focus fix using refs
// ──────────────────────────────────────────────
function PinInputRow({
  ...
  const handleChange = (i: number, raw: string) => {
    ...
    onChange(arr)
    if (digit && i < 5) {
      // Focus next — use timeout to let React flush state first
  ...
  const handlePaste = (e: React.ClipboardEvent) => {
    ...
    setTimeout(() => refs.current[nextEmpty]?.focus(), 0)
  }
...
// ──────────────────────────────────────────────
// Change PIN Dialog — Step-based UX
// ──────────────────────────────────────────────
function ChangePinDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // Auto-advance when current PIN is filled (6 digits → go to step new)
  useEffect(() => {
    if (step === 'current' && current.join('').length === 6) {
      // Small delay for UX feedback before advancing
      setTimeout(() => setStep('new'), 200)
    }
  }, [current, step])

  // Auto-submit when confirm PIN is filled
  useEffect(() => {
    if (step === 'new' && confirm.join('').length === 6) {
      setTimeout(() => handleSubmitNew(), 150)
    }
  }, [confirm, step])
  ...
  const handleSubmitNew = async () => {
    ...
    try {
      ...
      setNewError(msg)
      setNewP(Array(6).fill('')); setConfirm(Array(6).fill(''))
          ...
          {step === 'current' ? (
            <motion.div key="current" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
              <PinInputRow
                label="PIN ปัจจุบัน"
                value={current}
                onChange={val => { setCurrent(val); setCurrentError('') }}
                autoFocus
                error={currentError}
              />
              {current.join('').length === 6 && (
                <p className="mt-3 text-center text-xs text-stone-500 animate-pulse">กำลังดำเนินการ...</p>
              )}
            </motion.div>
          ) : (
...
export function AppHeader({ title, subtitle, actions }: AppHeaderProps) {
  ...
  // Fetch real notification data from dashboard
  ...
  return (
    <>
      <header className="flex h-16 flex-shrink-0 items-center gap-4 border-b border-white/10 bg-black/20 backdrop-blur-xl px-6 relative z-10">
        <div className="flex-1 min-w-0">
          {title && (
            <div>
              <h1 className="text-lg font-semibold text-stone-100 leading-tight truncate">{title}</h1>
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <span className="hidden lg:block text-xs text-stone-500">{today}</span>

          {/* Search */}
          <button onClick={() => setSearchOpen(true)}
            className="hidden md:flex items-center gap-2 rounded-full border border-white/15 bg-black/25 px-4 py-2 text-sm text-stone-500 backdrop-blur-xl hover:border-white/25 hover:text-stone-400 transition-colors">
            <Search className="h-3.5 w-3.5" />
            <span className="text-xs">ค้นหา...</span>
            <kbd className="ml-2 text-[10px] border border-white/10 rounded px-1">⌘K</kbd>
          </button>

          {/* Notifications — real data from dashboard API */}
          <Popover.Root>
            <Popover.Trigger asChild>
              <button className="relative flex h-9 w-9 items-center justify-center rounded-full border border-white/15 bg-white/[0.06] text-stone-400 hover:bg-white/[0.10] hover:text-stone-100 transition-colors">
          ...
          <DropdownMenu.Root>
            <DropdownMenu.Trigger asChild>
              <button className="flex items-center gap-2.5 rounded-full border border-white/15 bg-white/[0.06] pl-1 pr-3 py-1 text-sm hover:bg-white/[0.10] transition-colors outline-none">
                ...
                <span className="text-stone-300 text-xs max-w-[100px] truncate">{user?.firstName} {user?.lastName}</span>
            ...
            <DropdownMenu.Portal>
              <DropdownMenu.Content align="end" sideOffset={8}
                ...
                <DropdownMenu.Item
                  onSelect={() => setChangePinOpen(true)}
