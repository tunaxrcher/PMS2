      ...
      >
        ...
        {!value && !uploading && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-stone-600">
            ...
            {placeholder && <span className="text-[10px] text-stone-600">{placeholder}</span>}
            <span className="text-[10px]">JPEG, PNG, WebP สูงสุด 10MB</span>
          </div>
        )}
      </div>
          ...
          <div key={i} className="relative group h-24 w-32 rounded-xl overflow-hidden border border-white/15">
            ...
            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1">
              <button onClick={() => onSetPrimary(i)}
                className={cn('rounded-lg px-2 py-1 text-[10px] font-medium transition-colors', img.isPrimary ? 'bg-amber-400 text-stone-900' : 'bg-white/20 text-white hover:bg-amber-400/80 hover:text-stone-900')}>
            ...
            {img.isPrimary && (
              <div className="absolute top-1 left-1 rounded-full bg-amber-400 px-1.5 py-0.5 text-[9px] font-bold text-stone-900">หลัก</div>
            )}
          </div>
        ))}
